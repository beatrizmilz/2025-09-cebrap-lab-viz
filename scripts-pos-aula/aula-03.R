...
# lendo dos arquivos (backup)
estados <- read_rds("dados/geobr/estados.Rds")
View(estados)
# abrindo do geobr
estados <- read_state()
...
municipios_todos <- read_rds("dados/geobr/municipios_todos.rds")
glimpse(municipios_todos)
...
municipios_sp <- municipios_todos |>
...
write_rds(estado_df, file = "geobr_delimitacao_df.rds")
# útil para abrir no QGIS
sf::write_sf(estado_df, "output/geobr_delimitacao_df.shp")
...
ggplot() +
  ...
  geom_sf(data = estado_df, fill = "pink")
...
pnud_uf_sf |>
  # inicia o grafico
  ggplot() +
  # geometria de mapas, preencher com a coluna espvida
...
ggplot() +
  geom_sf(data = municipio_brasilia) +
  geom_sf(data = escolas_brasilia, aes(color = government_level), show.legend = FALSE, alpha = 0.5) +
  facet_wrap(~government_level) +
  theme_void() +
  labs(title = "Escolas em Brasília (2020) \n") +
...
class(dados_pnud_2010)

dados_pnud_2010 |>
  ggplot() +
  geom_sf()

dados_pnud_2010_sf <- dados_pnud_2010 |>
...
ggplot() +
  geom_sf(data = estados) +
  geom_sf(data = dados_pnud_2010_sf)
...
# sf::read_sf("arquivo.gpkg")


# escrever um geopackage
# sf::write_sf(objeto, "nome_arquivo.gpkg")
# sf::write_sf(objeto, "nome_arquivo.shp")

install.packages("leaflet")
library(leaflet)

# descobrir o nome dos providers
leaflet::providers

leaflet_pnud <- dados_pnud_2010 |>
  filter(uf_sigla == "SP") |>
  mutate(legenda = paste0(muni_nm, "<br> IDH municipal: ", idhm)) |> 
  leaflet() |>
  addProviderTiles("Esri.WorldImagery") |>
  addMarkers(popup = ~legenda)
  

htmlwidgets::saveWidget(widget = leaflet_pnud, file = "leaflet_pnud.html")


# dúvida dos conflitos
