...
# exemplo de quando temos valores faltantes
starwars |>
  ggplot() +
  aes(x = height, y = mass) + 
  geom_point()
...
media_idhm_por_regiao |> 
  filter(media_idhm == max(media_idhm))


# DUVIDA: SEM DESAGRUPAR
media_idhm_por_regiao_desagrupado <- dados_pnud_2010 |>
  group_by(regiao_nm, uf_sigla) |>
  summarise(media_idhm = mean(idhm)) 

# `summarise()` has grouped output by 'regiao_nm'. You can override using the `.groups` argument.

media_idhm_por_regiao_desagrupado |>
  filter(media_idhm == max(media_idhm))
...
media_idhm_por_regiao |>
  ggplot() + 
  aes(x = media_idhm, y = uf_sigla) +
  geom_col(aes(fill = regiao_nm), color = "black")
...
media_idhm_por_regiao |>
  ...
  aes(x = media_idhm, y = uf_sigla) +
  geom_col(aes(fill = regiao_nm)) +
  facet_wrap(vars(regiao_nm), scales = "free_y")
...
# Exemplo de erro: usamos pipe |> ao invés do +
dados_idhm_ano_regiao |>
  ggplot() +
  aes(x = ano, y = media_idhm) +
  geom_line(aes(color = regiao_nm, linetype = regiao_nm)) |>
...
dados_pnud_2010 |>
  ggplot() + 
  aes(x = idhm) +
  geom_histogram(binwidth = 0.01) # o valor vai depender da amplitude dos dados
...
dados_pnud_2010 |>
  ggplot() +
  aes(x = regiao_nm, y = espvida) +
  geom_boxplot(aes(fill = regiao_nm),
               color = "black",
               show.legend = FALSE)
